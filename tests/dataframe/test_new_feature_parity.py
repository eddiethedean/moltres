...
def test_polars_agg_dict_syntax_multiple_cols(sample_db):
    ...
    df_with_total = df.select(
        col("status"), col("amount"), (col("amount") * 1.1).alias("amount_with_tax")
    )

    # Group by status and aggregate with dict syntax
    result = df_with_total.group_by("status").agg({"amount": "sum", "amount_with_tax": "avg"})
    result_list = result.collect()
...
async def test_async_pandas_crossjoin(tmp_path):
    ...
    await db.create_table("products", [column("id", "INTEGER"), column("name", "TEXT")]).collect()

    await (await db.createDataFrame([{"id": 1, "name": "Alice"}], pk="id")).write.insertInto(
        "users"
    )
    await (await db.createDataFrame([{"id": 1, "name": "Product A"}], pk="id")).write.insertInto(
        "products"
    )
...
async def test_async_polars_semi_join(tmp_path):
    ...
    await (await db.createDataFrame([{"id": 1, "user_id": 1}], pk="id")).write.insertInto("orders")
...
async def test_async_pyspark_withcolumns(tmp_path):
    ...
    await db.create_table("users", [column("id", "INTEGER"), column("age", "INTEGER")]).collect()

    await (await db.createDataFrame([{"id": 1, "age": 30}], pk="id")).write.insertInto("users")
...
async def test_async_pandas_take(tmp_path):
    ...
    await (
        await db.createDataFrame([{"id": 1, "name": "Alice"}, {"id": 2, "name": "Bob"}], pk="id")
    ).write.insertInto("users")
