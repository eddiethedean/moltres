...
def execute_code_block(
    code: str, temp_dir: Path, context_vars: dict
    ...
    context_code = "\n".join(
        [
            f"{k} = {repr(v)}" if not isinstance(v, str) or "db" not in k else f"{k} = {v}"
    ...
    try:
        ...
        result = subprocess.run(
            ...
            cwd=temp_dir,
            env={
                **{k: str(v) for k, v in context_vars.items() if isinstance(v, (str, int, float))},
...
def process_guide_file(guide_path: Path, dry_run: bool = False) -> Tuple[int, int]:
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        for line_num, full_match, code in reversed(blocks):
            ...
            if len(code.split("\n")) < 3 and not any(
                kw in code for kw in ["print(", "collect()", "head(", "tail("]
            ):
                continue
...
def main():
    ...
    if args.guides:
        guide_files = [
            guides_dir / g if (guides_dir / g).exists() else Path(g) for g in args.guides
        ]
    ...
    print(f"\n{'=' * 70}")
    print(
        f"Summary: Updated {total_updated}/{total_blocks} code blocks across {len(guide_files)} guides"
    )
    if args.dry_run:
